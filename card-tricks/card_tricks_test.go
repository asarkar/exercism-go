...
func TestGetItem(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.want {
				t.Errorf(
					"GetItem(slice:%v, index:%v) got = %v, want %v",
					tt.args.slice,
					tt.args.index,
					got,
					tt.want,
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.args.slice) == len(got) {
				...
				if reflect.ValueOf(got).Pointer() != reflect.ValueOf(tt.args.slice).Pointer() {
					t.Errorf(
						"SetItem(slice:%v, index:%v) does not return the modified input slice)",
						tt.args.slice,
						tt.args.value,
					)
				}
...
func TestRemoveItem(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := RemoveItem(copySlice(tt.args.slice), tt.args.index); !slicesEqual(
				got,
				tt.want,
			) {
				t.Errorf(
					"RemoveItem(slice:%v, index:%v) = %v, want %v",
					tt.args.slice,
					tt.args.index,
					got,
					tt.want,
