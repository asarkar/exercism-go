	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(tt.myList, tt.expected) {
				t.Errorf(
					"addSecretIngredient(%v, %v) = %v want %v",
					tt.friendsList,
					myList,
					tt.myList,
					tt.expected,
			...
			if !reflect.DeepEqual(friendsList, tt.friendsList) {
				t.Errorf(
					"addSecretIngredient permuted friendsList (was %v, now %v), should not alter inputs",
					tt.friendsList,
					friendsList,
				)
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != len(tt.expected) {
				t.Errorf(
					"ScaleRecipe(%v, %d) produced slice of length %d, expected %d",
					inputList,
					tt.portions,
					len(got),
					len(tt.expected),
			...
			if !reflect.DeepEqual(inputList, tt.input) {
				t.Errorf(
					"ScaleRecipe permuted list (was %v, now %v), should not alter inputs",
					tt.input,
					inputList,
				)
			}
